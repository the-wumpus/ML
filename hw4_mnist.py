...
print("Loading the MNIST 784 dataset...")
mnist_sub = fetch_openml('mnist_784', version=1, cache=True)
X = mnist_sub.data
y = mnist_sub.target

# Split the dataset into training and testing sets, limit the training set to 10,000 samples
X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=10000, test_size=0.2, random_state=42, stratify=y)
...
# set number of components for testing
num_components = 2
print(f"Using {num_components} components for testing")
...
# Calculate the accuracy of the classifier to 2 decimal places
...
# measure the accuracy and testing time of PCA process
kpca = KPCA(n_components=num_components)
train_start_time = time.time()
